...
fs.readdirSync(root, { withFileTypes: true }).forEach((dirent) => {
  if (dirent.isDirectory()) {
    ...
    for (const [index, thread] of data.entries()) {
      ...
      execSync(`docker run --rm -v $PWD/data/json/${parentId}:/output tyrrrz/discordchatexporter:2.33.2 export \
  -t ${process.env.DISCORD_TOKEN} \
  -c ${threadId} \
  -o "/output/%P|${index}|%T|%C|%t|%c.json" \
  -f Json`);

      execSync(`docker run --rm -v $PWD/data/html/${parentId}:/output tyrrrz/discordchatexporter:2.33.2 export \
  -t ${process.env.DISCORD_TOKEN} \
  -c ${threadId} \
  -o "/output/%c.html" \
  -f HtmlLight`);
